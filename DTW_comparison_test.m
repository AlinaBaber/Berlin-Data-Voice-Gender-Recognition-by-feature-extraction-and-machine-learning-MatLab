function [Category,Emotion] = DTW_comparison_test(features)
 % Read speech samples, sampling rate and precision from file
DTWFile=load('dtwdata');
%features= allfeatures_extraction( wav_file );
    [NormalDist]=dtw(DTWFile.Normal_features,features);
    [SpecialDist]=dtw(DTWFile.Special_features,features);
    [AngryDist]=dtw(DTWFile.Angry_features,features);
    [HappyDist]=dtw(DTWFile.Happy_features,features);
    [NeutralDist]=dtw(DTWFile.Neutral_features,features);
    [SadDist]=dtw(DTWFile.Sad_features,features);
