...
% Last Modified by GUIDE v2.5 26-May-2017 18:39:08
...
handles.activex5.URL = FullFilePath;
...
% handles    structure with handles and user data (see GUIDATA)
wav_file=handles.sample_filepath_txt.String;
[Category,Category_score,Categories_error,Category_cost,Emotion,Emotion_score,Emotion_error,Emotion_cost] = knn_classification(wav_file);
C = get(handles.test_table, 'data');

Category_Score=num2cell(max(Category_score));
Categories_Error=num2cell(Categories_error);
Emotion_Score=num2cell(max(Emotion_score));
...
set(handles.test_table, 'data', C);
axes(handles.category_graph)
bar3(Category_output);
zlabel(' NN Score');
xlabel(' (1) Normal  (2) Special'); 
title('NN Language Disorder Result');
axes(handles.emotion_graph)
bar3(Emotions_output); 
zlabel(' NN Score');
xlabel('(1) Angry (2) Happy (3) Neutral (4) Sad'); 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= KNN_Test();

C = get(handles.algo_category_table, 'data');
C(2,1)= num2cell(Category_Error_Rate);%assign a cell array
C(2,2)= num2cell(Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
E = get(handles.algo_emotion_table, 'data');
E(2,1)= num2cell(Emotion_Error_Rate);%assign a cell array
E(2,2)= num2cell(Emotion_Accuracy_Rate);
set(handles.algo_emotion_table, 'data', E);
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(transpose(Categories),transpose(Emotions));
C = get(handles.error_table, 'data');
C(2,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
C(2,2)= num2cell(Normal_Error_Rate(1,2));
C(2,3)= num2cell(Normal_Error_Rate(1,3));
C(2,4)= num2cell(Normal_Error_Rate(1,4));
C(2,5)= num2cell(Special_Error_Rate(1,1));
C(2,6)= num2cell(Special_Error_Rate(1,2));
C(2,7)= num2cell(Special_Error_Rate(1,3));
C(2,8)= num2cell(Special_Error_Rate(1,4));
set(handles.error_table, 'data', C);
C = get(handles.performance_table, 'data');
C(2,1)= num2cell(Normal_Accuracy_Rate(1,1));%assign a cell array
C(2,2)= num2cell(Normal_Accuracy_Rate(1,2));
C(2,3)= num2cell(Normal_Accuracy_Rate(1,3));
C(2,4)= num2cell(Normal_Accuracy_Rate(1,4));
C(2,5)= num2cell(Special_Accuracy_Rate(1,1));
C(2,6)= num2cell(Special_Accuracy_Rate(1,2));
C(2,7)= num2cell(Special_Accuracy_Rate(1,3));
C(2,8)= num2cell(Special_Accuracy_Rate(1,4));
set(handles.performance_table, 'data', C);
...
hold on;
E = get(handles.algo_emotion_table, 'data');
subplot(2,1,2);bar33(E(1:5,:)); hold off;
